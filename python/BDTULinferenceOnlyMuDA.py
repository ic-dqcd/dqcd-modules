...
class DQCDULBDTOnlyMuDAProducer(JetLepMetSyst):
    def __init__(self, *args, **kwargs):
        ...
        if scenario == "A":
            default_model_path = os.path.expandvars(
                "$CMSSW_BASE/src/DQCD/Modules/data/ICEBOOST_DA_ONLYMU_149_scenarioA.model")
        elif scenario == "B1":
        ...
        super(DQCDULBDTOnlyMuDAProducer, self).__init__(*args, **kwargs)
        ...
        if not os.getenv("_DQCDBDTOnlyMuDA_%s" % self.model):
            os.environ["_DQCDBDTOnlyMuDA_%s" % self.model] = "_DQCDBDT_%s" % self.model
...
def DQCDULBDTOnlyMuDA(*args, **kwargs):
    """
    ...
    """
    return lambda: DQCDULBDTOnlyMuDAProducer(*args, **kwargs)
